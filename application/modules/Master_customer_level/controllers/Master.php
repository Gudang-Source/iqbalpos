...
class Master extends MX_Controller {
    ...
    function index(){
    	...
    	$data['list'] = json_encode($this->Customerlevelmodel->select($dataSelect, 'm_customer_level', 'date_add', 'DESC')->result());
		//echo $data;
		//print_r($data);
    	$this->load->view('Master_customer_level/view', $data);
    }
    ...
    function add(){
		...
		if($checkData->num_rows() < 1){
			...
			if($insert){
				$dataSelect['deleted'] = 1;
				$list = $this->Customerlevelmodel->select($dataSelect, 'm_customer_level', 'date_add', 'DESC')->result();
				echo json_encode(array('status' => 3,'list' => $list));
			}else{
