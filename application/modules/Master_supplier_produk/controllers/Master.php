...
class Master extends MX_Controller {
    private $modul = "Master_supplier_produk/";
    private $fungsi = "";    
    ...
    function _insertLog($fungsi = null){
        ...
        $insertLog = $this->Supplierprodukmodel->insert($dataInsert, 't_log');        
    }  
    function index(){
    	$dataSelect['deleted'] = 1;
        $data['list_prov'] = json_encode($this->Supplierprodukmodel->select($dataSelect, 'm_provinsi', 'nama')->result());
        $data['list_kota'] = json_encode($this->Supplierprodukmodel->select($dataSelect, 'm_kota', 'nama')->result());
    	$data['list'] = json_encode($this->Supplierprodukmodel->select($dataSelect, 'm_supplier_produk', 'date_add', 'DESC')->result());
		//echo $data;
		//print_r($data);
    	$this->load->view('Master_supplier_produk/view', $data);
    }
    ...
    function add(){
		...
		if($checkData->num_rows() < 1){
			...
			if($insert){
				$dataSelect['deleted'] = 1;
				$list = $this->Supplierprodukmodel->select($dataSelect, 'm_supplier_produk', 'date_add', 'DESC')->result();
				echo json_encode(array('status' => 3,'list' => $list));
			}else{
    ...
    function get_kota(){
    	...
    	$dataSelect['deleted'] = 1;
    	echo json_encode($this->Supplierprodukmodel->select($dataSelect, 'm_kota', 'nama')->result());
    }
