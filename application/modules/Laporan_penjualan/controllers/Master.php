...
class Master extends MX_Controller {
    ...
    private function data_per_hari() {
        ...
        $sql = "SELECT DATE(date_add) AS date, COUNT(id) AS jumlah_penjualan, SUM(grand_total) AS total_penjualan FROM t_order WHERE deleted = 1 AND (MONTH(date_add) = '".$cur_month."' AND YEAR(date_add) = '".$cur_year."') GROUP BY DATE(date_add)";
    ...
    private function data_per_bulan() {
        ...
        $sql = "SELECT MONTH(date_add) AS month, COUNT(id) AS jumlah_penjualan, SUM(grand_total) AS total_penjualan FROM t_order WHERE deleted = 1 AND (YEAR(date_add) = '".$cur_year."') GROUP BY MONTH(date_add)";
        $result = $this->Laporanpenjualanmodel->rawQuery($sql)->result();
