...
class Master extends MX_Controller {
    ...
    function index(){
        ...
        $data['list_prov'] = json_encode($this->Supplierbahanmodel->select($dataSelect, 'm_provinsi')->result());
        $data['list_kota'] = json_encode($this->Supplierbahanmodel->select($dataSelect, 'm_kota')->result());
    	$data['list'] = json_encode($this->Supplierbahanmodel->select($dataSelect, 'm_supplier_bahan')->result());
		//echo $data;
		//print_r($data);
    	$this->load->view('Supplier_bahan/view', $data);
    }
	
	function test(){
		header('Content-Type: application/json; charset=utf-8');
		$dataSelect['deleted'] = 1;
		$list = $this->Supplierbahanmodel->select($dataSelect, 'm_supplier_bahan')->result();
    ...
    function get_kota(){
    	...
    	$dataSelect['deleted'] = 1;
    	echo json_encode($this->Supplierbahanmodel->select($dataSelect, 'm_kota')->result());
    }
